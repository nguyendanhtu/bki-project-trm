...
public partial class ChucNang_F405_ChinhSuaXacNhanNganHang : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            lblUser.ToolTip =CIPConvert.ToStr(ID_TRANG_THAI_THANH_TOAN.NGAN_HANG_CHUYEN_KHOAN_THANH_CONG);
            m_lbl_thong_bao0.ToolTip = CIPConvert.ToStr(ID_TRANG_THAI_THANH_TOAN.NGAN_HANG_CHUYEN_KHOAN_KHONG_THANH_CONG);
    ...
    private void load_data_2_grid(string ip_str_ma_dot_tt)
    {
        ...
        else
        {
            ...
            v_us_gd_thanh_toan.f501_load_thanh_toan_by_ma_dot_tt_va_trang_thai_tt(ip_str_ma_dot_tt, CIPConvert.ToDecimal(lblUser.ToolTip), v_ds_gd_thanh_toan, CIPConvert.ToStr(Session["user_quyen"]));
            //v_us_gd_thanh_toan.FillDataset(v_ds_gd_thanh_toan, " WHERE SO_PHIEU_THANH_TOAN = '" + ip_str_ma_dot_tt + "' AND ID_TRANG_THAI_THANH_TOAN = " + CIPConvert.ToDecimal(lblUser.ToolTip) + " ORDER BY ID");
    ...
    private void load_data_2_grid_search(string ip_str_ma_dot_tt, string ip_str_so_hd)
    {
        ...
        //    return;
        //}
        v_us_gd_thanh_toan.f403_load_thanh_toan_by_ma_dot_tt_va_trang_thai_tt_va_like_so_hd(ip_str_ma_dot_tt, CIPConvert.ToDecimal(lblUser.ToolTip), ip_str_so_hd, v_ds_gd_thanh_toan, CIPConvert.ToStr(Session["user_quyen"]));
    ...
    private void us_obj_2_form(US_V_GD_THANH_TOAN ip_us_gd_thanh_toan)
    {
        ...
        m_cbo_trang_thai_thanh_toan.SelectedValue = m_lbl_thong_bao0.ToolTip;
    ...
    private decimal get_id_trang_thai_ngan_hang_chuyen_khoan_khong_thanh_cong()
    {
        ...
        if (v_ds_tu_dien.CM_DM_TU_DIEN.Rows.Count == 0) return 515;
        return CIPConvert.ToDecimal(v_ds_tu_dien.CM_DM_TU_DIEN.Rows[0][CM_DM_TU_DIEN.ID]);
    }
    ...
    protected void m_cmd_cap_nhat_du_toan_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            m_us_v_gd_thanh_toan.update_xac_nhan_ngan_hang();
            reset_controls();
            m_cmd_cap_nhat_du_toan.Enabled = true;
            m_cbo_dot_thanh_toan.Enabled = true;
            load_data_2_grid(get_ma_dot_tt_by_id_dot(CIPConvert.ToDecimal(m_cbo_dot_thanh_toan.SelectedValue)));
            m_lbl_thong_bao.Text = "Đã chỉnh sửa xác nhận ngân hàng về chưa có xác nhận của ngân hàng";
        }
        catch (Exception v_e)
        {
            CSystemLog_301.ExceptionHandle(this, v_e);
